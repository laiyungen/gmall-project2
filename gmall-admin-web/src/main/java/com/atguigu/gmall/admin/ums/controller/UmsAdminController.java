...
/**
 * 后台用户管理
 * SpringMVC 支持使用 【JSR303】 方式进行校验
 * 1、springboot 默认导第三方的校验框架 hibernate-validator
 *
 * 使用 JSR303 的三大步
 * 1）、给需要校验数据的 javaBean 上标注校验注解；
 * 2）、告诉 SpringBoot，这个需要校验；@Valid
 *      springmvc 进入方法之前，确定参数值的时候就会进行校验，
 *      如果校验出错，直接返回错误，不执行controller代码
 * 3）、如何感知校验成功还是失败；
 *      只需要给开启了校验的 javaBean 参数后面，紧跟一个 BindingResult 对象就可以获取到校验结果;
 *         只要有 BindingResult，即使校验错了，方法也会执行。我们需要手动处理
 *
 * 统一的异常处理：
 *
 *
...
public class UmsAdminController {
    ...
    public Object getAdminInfo(HttpServletRequest request) {
        ...
        //1、getOne 是 mybatis-plus 生成的，而且带了泛型的。
        //2、dubbo 没办法直接调用 mybatis-plus 中带泛型的 service；
        //3、实战经验：mybatis-plus 自动生成有可能有兼容问题，最好不要远程调用；
